...
/**
 * Practice Mode player — Slices 1 + 2. Pick any library song that has audio and
 * play it with a slow-down (1.0 / 0.75 / 0.5, pitch preserved) + scrubber; jump
 * between section markers (per-song, reusable; Ar manages); loop a section A→B; and
 * run a break timer. Single device, online: audio streams from R2 on demand.
 */
export function PracticeMode({
  ...
  // markers (per song) — seeded from the server, mutated locally on add/delete
  ...
  // A-B loop (client-side only, per the loaded track)
  const [loopA, setLoopA] = useState<number | null>(null);
  const [loopB, setLoopB] = useState<number | null>(null);
  const [loopOn, setLoopOn] = useState(false);
  // refs so the (stable) timeupdate listener can read the live loop window
  ...
  useEffect(() => {
    ...
    const onTime = () => {
      ...
      // A-B loop: jump back to A when we pass B
  ...
  async function selectSong(song: Song) {
    ...
    try {
      ...
      // a new track invalidates the previous A-B window
      setLoopA(null);
      setLoopB(null);
      setLoopOn(false);
  ...
  async function addMarker(label: string) {
    ...
    const { data, error } = await supabase
      ...
      .single();
    if (error || !data) {
      toast.error("เพิ่มมาร์คไม่สำเร็จ", {
      ...
      <div className="flex flex-wrap items-end justify-between gap-2">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            โหมดซ้อม — เลือกเพลงจากคลัง, ปรับความเร็ว, วนท่อน
          </p>
        </div>
        <BreakTimer />
      </div>
