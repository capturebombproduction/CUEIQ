...
const NEXT_REDACTED_PREFIX = "An error occurred in the Server Components render";

/**
 * The body of every non-global error boundary in the web app.
 *
 * Extracted so `app/error.tsx` and `app/(app)/error.tsx` cannot drift. They are two
 * boundaries because Next never lets an error.tsx catch a throw from the layout.tsx
 * BESIDE it — and round 11 made that distinction load-bearing: getWorkspace() now
 * throws on a failed read rather than answering "you are not in a band", and it is
 * called from `app/(app)/layout.tsx`, so that throw goes past the in-app boundary
 * to whatever sits above it. Without a root `app/error.tsx` the user lands on
 * global-error, which replaces the root layout wholesale and cannot say the one
 * thing they need to hear — that nothing was lost.
 */
export function ErrorCard({
  ...
  const redacted =
    typeof error.message === "string" && error.message.startsWith(NEXT_REDACTED_PREFIX);
  const digestLine = error.digest ? `digest: ${error.digest}` : "";
  const details =
    (redacted ? digestLine : [error.message, digestLine].filter(Boolean).join("\n\n")) ||
    "ไม่ทราบสาเหตุ";
