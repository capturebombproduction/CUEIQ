...
import { Play, Pause, SkipForward, SkipBack, RotateCcw, Radio } from "lucide-react";
...
interface LiveState {
  running: boolean;
  startedAt: number | null; // epoch ms of show start
  itemStartedAt: number | null; // epoch ms current item started
  currentIndex: number;
}
    ...
    <div className="mx-auto max-w-2xl space-y-4">
        ...
        {current && current.mic_slots?.length > 0 && (
          <div className="mt-4 flex flex-wrap justify-center gap-1.5">
            {current.mic_slots.map((s, i) => (
              <Badge key={i} variant="outline" className="border-black/20 bg-black/5">
                {s.mic} → {s.member}
              </Badge>
            ))}
          </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="rounded-xl border bg-card p-4 text-center">
          ...
          <p className="truncate text-lg font-semibold">{next?.title || "— จบโชว์ —"}</p>
        </div>
      </div>
      ...
      <div className="rounded-xl border bg-card">
        {items.map((it, i) => (
          <button
            ...
            onClick={() => goto(i)}
            className={cn(
              "flex w-full items-center gap-3 border-b px-4 py-2.5 text-left text-sm last:border-0 hover:bg-muted/50",
              i === state.currentIndex && "bg-primary/10 font-medium"
            )}
          >
