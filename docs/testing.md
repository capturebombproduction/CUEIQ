...
เจ็ดสถานการณ์ (สองอันรันแอป **สองตัวพร้อมกัน**, หนึ่งอัน**วัดเสียงจริง**):
...
| **`audible`** | ปลูกไฟล์ WAV จริง (คลื่นไซน์ 440 Hz) ลงแคชเสียงของแอปเอง แล้วเริ่มโชว์ | เพลงเล่นจริง **และมีคลื่นเสียงออกมาจริง** — วัด RMS จาก AnalyserNode ต้อง > 0.05 |
...
`audible` คือข้อที่เคยต้องใช้**หูคน**เท่านั้น กลไก:

- สร้างไฟล์ WAV ขึ้นเองในโค้ด (ไม่ commit ไฟล์เสียง — ไฟล์ที่ไม่มีใครเปิดอ่านซ้ำ ถ้ามันเงียบขึ้นมาจะกลายเป็น
  "แอปพัง" ทั้งที่แอปปกติ) แล้วปลูกลง IndexedDB `cueiq-audio` ตามสคีมาของ `lib/audio-store.ts` **ก่อน**เปิดหน้า Live
  (การกู้ไฟล์เป็น mount effect ปลูกทีหลัง = สาย) และต้องเป็น **Blob ไม่ใช่ File** เพราะ live-mode ถือว่า File = ลิงก์ไปไฟล์บนดิสก์ที่อาจหายไป แล้วจะไปดึงของใหม่ทับ
- ⚠️ **`<audio>` ของ Live Mode ไม่ได้อยู่ใน DOM** — สร้างด้วย `new Audio()` แล้วไม่เคย append
  `querySelectorAll("audio")` จึงหาไม่เจอ ต้อง**ดักที่ตัวสร้าง** (patch `window.Audio` ผ่าน CDP ก่อนหน้าโหลด)
- ⚠️ **ต้องกดปุ่มด้วย `userGesture: true`** ไม่งั้น autoplay policy ของ Chromium บล็อกเสียง — `.click()` เฉย ๆ ไม่นับเป็นการกดของคน
- วัดด้วย AnalyserNode ต่อจาก `createMediaElementSource` → ต่อ `ctx.destination` ต่อ (ไม่ต่อ = การวัดจะไปปิดเสียงแอปเสียเอง)

**พิสูจน์แล้วว่ามันแดงได้จริง**: เปลี่ยน amplitude ของไฟล์เป็น 0 แล้วรันใหม่ → **ทุกค่าอื่นยังเขียวหมด**
(playing=true, playhead เดิน 1.28 วิ, volume 1, readyState 4) มีแต่ RMS ที่เป็น 0 แล้วเทสแดง —
นั่นคือเหตุผลที่ต้องวัดคลื่น ไม่ใช่แค่เช็คว่า "สั่งเล่นแล้ว"
...
⚠️ **ตัวขับ scenario อยู่ใน `main.cjs` = อยู่ใน "ตัวแอป"** ดังนั้น scenario ที่เพิ่งเขียนใหม่
**รันกับ .exe เวอร์ชันเก่าที่ติดตั้งไว้แล้วไม่ได้** (มันจะตอบว่า `CUEIQ_SMOKE_LIVE must be …`)
เป็นเรื่องปกติ ไม่ใช่บั๊ก — ใช้โหมด `--app` หรือรอ build ของ tag ถัดไป
...
- jsdom ไม่ decode เสียง ไม่บังคับ autoplay policy — เทสต์ฝั่งนั้นพิสูจน์ *เจตนา*
  (สั่งถูกคำสั่ง, mute ถูกตัว, ส่ง prop ถูก) ไม่ใช่ *ฟิสิกส์*
- `audible` **วัดในตัว Chromium ก่อนถึงมิกเซอร์ของ Windows** — โดยตัวมันเองพูดได้แค่ว่า
  "แอปผลิตสัญญาณเสียงจริง" บอกไม่ได้ว่าลำโพงเสียบอยู่หรือเปิดอยู่ คำถามที่เหลือให้คนตอบจึงเปลี่ยนจาก
  "แอปทำงานไหม" เป็น "สายเสียบอยู่ไหม"
  ✅ **แต่ห่วงโซ่ทั้งเส้นเคยเดินครบแล้วหนึ่งครั้ง (2026-08-16):** ระหว่างรัน `audible` บนเครื่องพัฒนา
  (Windows, CueIQ 0.1.10 ที่ติดตั้งจาก Release จริง) **พี่พัชร์ได้ยินเสียง "ตี๊ดดด" โทนเดียวยาว ๆ ออกลำโพงหลายรอบ**
  — นั่นคือคลื่นไซน์ 440 Hz ที่ `makeToneWav()` สร้างขึ้นเอง ไม่มีที่มาอื่น และรอบที่ปลูกไฟล์เงียบก็ไม่ดัง
  ตรงตามที่ควรเป็น จึงยืนยันได้ว่า ไฟล์ → decode → Chromium → มิกเซอร์ Windows → ลำโพงจริง ครบทั้งเส้น
